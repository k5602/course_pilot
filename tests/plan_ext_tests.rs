...
use course_pilot::types::{Plan, PlanSettings, PlanItem, PlanExt, PlanItemIdentifier};

fn create_test_plan() -> Plan {
    let mut plan = Plan::new(Uuid::new_v4(), PlanSettings {
        start_date: Utc::now(),
        sessions_per_week: 3,
...
fn test_calculate_progress_empty_plan() {
    let plan = Plan::new(Uuid::new_v4(), PlanSettings {
        start_date: Utc::now(),
        sessions_per_week: 3,
        session_length_minutes: 60,
        include_weekends: false,
