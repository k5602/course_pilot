...
async fn test_plan_item_completion_toggle() {
    ...
    let mut plan = Plan::new(course.id, PlanSettings {
        start_date: Utc::now(),
        sessions_per_week: 3,
    ...
    backend.update_plan_item_completion(plan.id, 0, true).await.unwrap();
    
    let updated_plan = backend.get_plan_by_course(course.id).await.unwrap().unwrap();
    assert!(updated_plan.items[0].completed);
    
    // Test toggle back
    backend.update_plan_item_completion(plan.id, 0, false).await.unwrap();
...
async fn test_plan_progress_calculation() {
    ...
    // Setup test data
    let course = Course::new("Test Course".to_string(), vec!["Video 1".to_string(), "Video 2".to_string()]);
    backend.create_course(course.clone()).await.unwrap();
    ...
    let course_progress = backend.get_course_progress(course.id).await.unwrap().unwrap();
...
async fn test_plan_item_out_of_bounds() {
    ...
    let plan = Plan::new(course.id, PlanSettings {
        start_date: Utc::now(),
        sessions_per_week: 3,
        session_length_minutes: 60,
        include_weekends: false,
...
async fn test_nonexistent_plan() {
    ...
    let result = backend.update_plan_item_completion(fake_plan_id, 0, true).await;
    assert!(result.is_err());
}
