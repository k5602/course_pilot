...
async fn test_directory_scanning_performance() {
    ...
    assert!(scan_time < Duration::from_secs(5), "Scanning should complete within 5 seconds");
}
...
async fn test_database_operations_performance() {
    ...
    println!("Time to create 10 plans with 50 items each: {:?}", plan_ops_time);
    ...
    println!("Time to calculate progress for 10 courses: {:?}", progress_calc_time);
    
    // Performance assertions
    assert!(course_creation_time < Duration::from_secs(10), "Course creation should be fast");
    assert!(course_loading_time < Duration::from_secs(1), "Course loading should be very fast");
    assert!(plan_ops_time < Duration::from_secs(5), "Plan operations should be reasonably fast");
    assert!(progress_calc_time < Duration::from_secs(2), "Progress calculation should be fast");
}
...
async fn test_concurrent_operations_performance() {
    ...
    println!("Time for 20 concurrent course creations: {:?}", concurrent_time);
    ...
    let course = Course::new("Concurrent Plan Test".to_string(), vec!["Video 1".to_string()]);
    ...
    for i in 0..10 {
        ...
        update_set.spawn(async move {
            backend_clone.update_plan_item_completion(plan_id, i, true).await.unwrap();
        });
    }
    ...
    println!("Time for 10 concurrent plan item updates: {:?}", update_time);
    ...
    // Performance assertions
    assert!(concurrent_time < Duration::from_secs(15), "Concurrent operations should complete reasonably fast");
    assert!(update_time < Duration::from_secs(5), "Concurrent updates should be fast");
}
...
async fn test_memory_usage_with_large_datasets() {
    ...
    let course = Course::new("Memory Test Course".to_string(), vec!["Video 1".to_string()]);
    let course_id = course.id;
    backend.create_course(course).await.unwrap();
    ...
    let loaded_plan = backend.get_plan_by_course(course_id).await.unwrap().unwrap();
    ...
    println!("Time to calculate progress for 1000 items: {:?}", progress_time);
    
    // Performance assertions for large datasets
    assert!(large_plan_time < Duration::from_secs(10), "Large plan save should complete within 10 seconds");
    assert!(load_time < Duration::from_secs(5), "Large plan load should complete within 5 seconds");
    assert!(progress_time < Duration::from_secs(1), "Progress calculation should be fast even for large plans");
}