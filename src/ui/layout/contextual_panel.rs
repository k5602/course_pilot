...
const CONTEXT_PANEL_BG: &str = "bg-base-200 bg-opacity-90 backdrop-blur-md border-l border-base-300";
...
pub fn ContextualPanel() -> Element {
    ...
    let panel_style = use_memo(move || {
        format!("transform: translateX({}%);", panel_x.get_value())
    });
...
fn render_tab_content(active_tab: ContextualPanelTab, course_id: Option<uuid::Uuid>) -> Element {
    match active_tab {
        ContextualPanelTab::Notes => rsx!(NotesPanel { course_id: course_id }),
