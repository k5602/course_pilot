...
impl Exportable for Vec<Note> {
    ...
    fn export_csv(&self) -> Result<String> {
        ...
        // CSV Header
        csv_data.push_str("Note_ID,Course_ID,Video_ID,Content,Tags,Timestamp_Seconds,Created_At,Updated_At\n");
        
        for note in self {
            let video_id_str = note.video_id
                .map(|id| id.to_string())
                .unwrap_or_else(|| "".to_string());
            
            let tags_str = note.tags.join(";");
            
            let timestamp_str = note.timestamp
                .map(|ts| ts.to_string())
                .unwrap_or_else(|| "".to_string());
...
impl NotesExportExtensions for &[Note] {
    ...
    fn generate_pdf_content(&self) -> Result<String> {
        ...
        content.push_str(&format!("Average Note Length: {:.1} characters\n", statistics.average_note_length));
        ...
        for note in *self {
            notes_by_course.entry(note.course_id).or_insert_with(Vec::new).push(note);
        }
        
        // Export notes grouped by course
        for (course_id, course_notes) in notes_by_course {
            content.push_str(&format!("COURSE: {}\n", course_id));
            ...
            for note in sorted_notes {
                ...
                if let Some(timestamp) = note.timestamp {
                    ...
                    content.push_str(&format!("Timestamp: {} ({})\n", timestamp, utils::format_duration(duration)));
                }
    ...
    fn export_markdown(&self) -> Result<String> {
        ...
        markdown.push_str(&format!("- **Course-level Notes:** {}\n", statistics.course_level_notes));
        markdown.push_str(&format!("- **Video-level Notes:** {}\n", statistics.video_level_notes));
        markdown.push_str(&format!("- **Notes with Timestamps:** {}\n", statistics.notes_with_timestamps));
        markdown.push_str(&format!("- **Average Note Length:** {:.1} characters\n", statistics.average_note_length));
        ...
        // Export notes grouped by course
        for (course_id, course_notes) in notes_by_course {
            markdown.push_str(&format!("## Course: {}\n\n", course_id));
...
pub mod notes_utils {
    ...
    pub fn export_notes_by_tags(notes: &[Note], tags: &[String], options: ExportOptions) -> Result<ExportResult> {
        ...
        if filtered_notes.is_empty() {
            return Err(anyhow::anyhow!("No notes found with the specified tags: {}", tags.join(", ")));
        }
    ...
    pub fn export_timestamped_notes(notes: &[Note], options: ExportOptions) -> Result<ExportResult> {
    ...
    pub fn generate_notes_summary(notes: &[Note]) -> Result<String> {
        ...
        summary.push_str(&format!("Average Note Length: {:.1} characters\n\n", statistics.average_note_length));
