...
impl Exportable for Plan {
    ...
    fn export_csv(&self) -> Result<String> {
        ...
        // CSV Header
        csv_data.push_str("Date,Module,Section,Video_Indices,Completed,Session_Number,Week_Number\n");
        ...
        for (index, item) in self.items.iter().enumerate() {
            ...
            let video_indices_str = item.video_indices
                .iter()
                .map(|i| i.to_string())
                .collect::<Vec<_>>()
                .join(";");
...
impl Plan {
    ...
    fn generate_pdf_content(&self) -> Result<String> {
        ...
        content.push_str("-------------\n");
        content.push_str(&format!("Start Date: {}\n", self.settings.start_date.format("%Y-%m-%d")));
        content.push_str(&format!("Sessions per Week: {}\n", self.settings.sessions_per_week));
        content.push_str(&format!("Session Length: {} minutes\n", self.settings.session_length_minutes));
        content.push_str(&format!("Include Weekends: {}\n\n", if self.settings.include_weekends { "Yes" } else { "No" }));
        
        // Progress summary
        content.push_str("PROGRESS SUMMARY\n");
        content.push_str("----------------\n");
        content.push_str(&format!("Total Sessions: {}\n", progress_summary.total_sessions));
        content.push_str(&format!("Completed Sessions: {}\n", progress_summary.completed_sessions));
        ...
        for (index, item) in self.items.iter().enumerate() {
            ...
            let video_indices = item.video_indices
                .iter()
                .map(|i| i.to_string())
                .collect::<Vec<_>>()
                .join(", ");
            ...
            if !video_indices.is_empty() {
                content.push_str(&format!("   Videos: {}\n", video_indices));
            }
            content.push_str("\n");
        }
...
pub mod plan_utils {
    ...
    pub fn export_plan_calendar(plan: &Plan) -> Result<String> {
        ...
        for (index, item) in plan.items.iter().enumerate() {
            ...
            let end_time = (item.date + chrono::Duration::minutes(plan.settings.session_length_minutes as i64))
            ...
            ical_content.push_str(&format!("DESCRIPTION:Module: {}\\nSection: {}\n", 
    ...
    pub fn generate_plan_statistics(plan: &Plan) -> Result<String> {
        ...
        // Basic statistics
        stats.push_str(&format!("Total Sessions Planned: {}\n", progress_summary.total_sessions));
        stats.push_str(&format!("Sessions Completed: {}\n", progress_summary.completed_sessions));
        stats.push_str(&format!("Completion Rate: {:.1}%\n", progress_summary.progress_percentage));
        
        // Time analysis
        let total_planned_minutes = progress_summary.total_sessions as u32 * progress_summary.average_session_length;
        let completed_minutes = progress_summary.completed_sessions as u32 * progress_summary.average_session_length;
        
        stats.push_str(&format!("Total Planned Time: {} hours\n", total_planned_minutes / 60));
        stats.push_str(&format!("Time Completed: {} hours\n", completed_minutes / 60));
        stats.push_str(&format!("Time Remaining: {} hours\n", (total_planned_minutes - completed_minutes) / 60));
        
        // Weekly analysis
        stats.push_str(&format!("Sessions per Week: {}\n", progress_summary.sessions_per_week));
        stats.push_str(&format!("Weekly Time Commitment: {} hours\n", 
            (progress_summary.sessions_per_week as u32 * progress_summary.average_session_length) / 60));
        
        if let Some(completion_date) = progress_summary.estimated_completion_date {
            stats.push_str(&format!("Estimated Completion: {}\n", completion_date.format("%Y-%m-%d")));
        }
